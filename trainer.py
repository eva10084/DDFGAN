...
class PCH:
    ...
    def compute_losses(self):
        ...
        l2_loss_b = tf.add_n(
            [0.0001 * tf.nn.l2_loss(v) for v in tf.trainable_variables() if '/s_B/' in v.name or '/e_B/' in v.name])
        ...
        self.g_A_trainer = optimizer.minimize(g_loss_A,
                                              var_list=de_A_vars + de_c_vars + e_c_vars + e_cs_vars + e_dB_vars)
        self.g_B_trainer = optimizer.minimize(g_loss_B,
                                              var_list=de_B_vars + de_c_vars + e_c_vars + e_ct_vars + e_dA_vars)
        ...
        self.s_B_loss_merge_summ = tf.summary.merge(
            [self.ce_B_loss_summ, self.dice_B_loss_summ, self.l2_B_loss_summ, self.s_B_loss_summ, self.s_A_loss_summ])
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            for epoch in range(sess.run(self.global_step), self._max_step):
                ...
                for i in range(0, max_inter):
                    ...
                    summary_str_gan, summary_str_seg, summary_str_lossf = sess.run(
                        [self.lr_gan_summ, self.lr_seg_summ, self.loss_f_weight_summ],
                        feed_dict={
                            self.learning_rate: curr_lr,
                            self.learning_rate_seg: curr_lr_seg,
                            self.loss_f_weight: loss_f_weight_value,
                ...
                for m in range(0, max_inter_val):
                    np_b_mean = sess.run([self.dice_b_mean], feed_dict={self.input_b: inputs_val['images_j_val'],
                                                                        self.gt_b: inputs_val['gts_j_val'],
                                                                        self.is_training: False, self.keep_rate: 1.0})
...
def main(log_dir,  checkpoint_dir, skip, kfold):
    ...
    SAVE_DIR = prefix + '/save_param'   # 保存参数路径
    ...
    # DA_optim = torch.optim.Adam([{'params': pch_model.parameters()}, lr=LR, weight_decay=WEIGHT_DECAY)
...
if __name__ == '__main__':
    # i for K_fold
    # for i in range(0, 2):
    main(log_dir='output',  checkpoint_dir='', skip=True, kfold=1)
