...
def get_outputs(inputs, skip=False, is_training=True, keep_rate=0.75):
    ...
    # 创建一个TensorFlow变量作用域(variable scope)，名称为"Model"，并且设置reuse参数为tf.AUTO_REUSE。
    with tf.variable_scope("Model", reuse=tf.AUTO_REUSE) as scope:
        # 引用了下列新定义的各种网络模块，改名
        current_discriminator = discriminator   # 判别器
        ...
        fake_images_tmp_b=  current_decoderc(tf.concat([latent_a,latent_b_diff], axis=3),  name='de_c', skip=skip)
        fake_images_tmp_a = current_decoderc(tf.concat([latent_b,latent_a_diff], axis=3),  name='de_c', skip=skip)
        ...
        cycle_images_tmp_b=  current_decoderc(tf.concat([latent_fake_a,latent_fb_diff], axis=3), 'de_c', skip=skip)
        cycle_images_tmp_a = current_decoderc(tf.concat([latent_fake_b,latent_fa_diff], axis=3), 'de_c', skip=skip)
...
def build_encoderc(inputen, name='encoder', is_training=True, skip=False, keep_rate=0.75):
    with tf.variable_scope(name):
        fb = 16
        padding = "CONSTANT"
...
def build_encoders(o_r12, name='encoder', is_training=True, skip=False, keep_rate=0.75):
    with tf.variable_scope(name):
        fb = 16
        padding = "CONSTANT"
...
def build_encodert(o_r12, name='encoder', is_training=True, skip=False, keep_rate=0.75):
    with tf.variable_scope(name):
        fb = 16
        padding = "CONSTANT"
...
def build_encoderdiffa(inputen, name='encoder', is_training=True, skip=False, keep_rate=0.75):
    with tf.variable_scope(name):
        fb = 8
        k1 = 3
        padding = "CONSTANT"
...
def discriminator(inputdisc, name="discriminator"):  # 判别器
    with tf.variable_scope(name):
        f = 4
...
# inputdisc 输入图
def discriminator_aux(inputdisc, name="discriminator"):   # 加aux的判别器
    with tf.variable_scope(name):
        f = 4    # 卷积核大小
        padw = 2   # 填充大小
        ...
        # 对输入进行填充
        pad_input = tf.pad(inputdisc, [[0, 0], [padw, padw], [padw, padw], [0, 0]], "CONSTANT")
        # 第一个卷积层，不使用归一化，使用LeakyReLU作为激活函数
        o_c1 = layers.general_conv2d(pad_input, ndf, f, f, 2, 2, 0.02, "VALID", "c1", do_norm=False, relufactor=0.2, norm_type='Ins')

        # 对第一层卷积结果进行填充
        pad_o_c1 = tf.pad(o_c1, [[0, 0], [padw, padw], [padw, padw], [0, 0]], "CONSTANT")
        # 第二个卷积层，使用Insnorm作为归一化，使用LeakyReLU作为激活函数
        o_c2 = layers.general_conv2d(pad_o_c1, ndf * 2, f, f, 2, 2, 0.02, "VALID", "c2", relufactor=0.2, norm_type='Ins')

        # 对第二层卷积结果进行填充
        pad_o_c2 = tf.pad(o_c2, [[0, 0], [padw, padw], [padw, padw], [0, 0]], "CONSTANT")
        # 第三个卷积层，使用Insnorm作为归一化，使用LeakyReLU作为激活函数
        o_c3 = layers.general_conv2d(pad_o_c2, ndf * 4, f, f, 2, 2, 0.02, "VALID", "c3", relufactor=0.2, norm_type='Ins')

        # 对第三层卷积结果进行填充
        pad_o_c3 = tf.pad(o_c3, [[0, 0], [padw, padw], [padw, padw], [0, 0]], "CONSTANT")
        # 第四个卷积层，使用Insnorm作为归一化，使用LeakyReLU作为激活函数
        o_c4 = layers.general_conv2d(pad_o_c3, ndf * 8, f, f, 1, 1, 0.02, "VALID", "c4", relufactor=0.2, norm_type='Ins')

        # 对第四层卷积结果进行填充
        pad_o_c4 = tf.pad(o_c4, [[0, 0], [padw, padw], [padw, padw], [0, 0]], "CONSTANT")
        # 最后一层卷积，输出结果为2，不使用归一化和ReLU激活函数
        o_c5 = layers.general_conv2d(pad_o_c4, 2, f, f, 1, 1, 0.02, "VALID", "c5", do_norm=False, do_relu=False)

        # 第一个张量是o_c5的第一个通道的扩展维度，第二个张量是o_c5的第二个通道的扩展维度。这两个张量可以作为GAN的辅助分类器输出结果。
